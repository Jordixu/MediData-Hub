...
class DoctorAppointments(ctk.CTkFrame):
    def __init__(self, parent, controller):
        super().__init__(parent)
        self.controller = controller
        self.title = "Doctor Appointments"
    ...
    def load_appointments(self):
        ...
        for appointment_id in doctor_data.get_protected_attribute("appointments"):
            ...
            try:
                ...
                pat = appointment.get("patient_hid", "N/A")
                ...
                item_id = self.tree.insert(
                    "", "end", 
                    values=(appt_id, date, time, pat, room, status)
                )
    ...
    def cancel_appointment(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm Cancellation", 
            f"Are you sure you want to cancel appointment {appointment_values[0]} on {appointment_values[1]}?\n\nThis action cannot be undone."
        )
        
        if confirm:
            try:
                self.controller.hospital.cancel_appointment(int(appointment_values[0]), "doctor")
                messagebox.showinfo("Success", "The appointment has been canceled successfully.")
