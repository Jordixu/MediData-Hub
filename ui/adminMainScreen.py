...
class AdminMainScreen(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkButton(button_frame, text="Patients", width=200,
            command=lambda: controller.show_frame("AdminPatients")).grid(row=0, column=0, padx=10, pady=10)
        ctk.CTkButton(button_frame, text="Doctors", width=200,
            command=lambda: controller.show_frame("AdminDoctors")).grid(row=0, column=1, padx=10, pady=10)
        ctk.CTkButton(button_frame, text="Drugs", width=200,
            command=lambda: controller.show_frame("AdminDrugs")).grid(row=0, column=2, padx=10, pady=10)
        ctk.CTkButton(button_frame, text="Appointments", width=200,
            command=lambda: controller.show_frame("AdminAppointments")).grid(row=1, column=0, padx=10, pady=10)
        ctk.CTkButton(button_frame, text="Notifications", width=200,
            command=lambda: controller.show_frame("AdminNotifications")).grid(row=1, column=1, padx=10, pady=10)
        ctk.CTkButton(button_frame, text="Rooms", width=200,
            command=lambda: controller.show_frame("AdminRooms")).grid(row=1, column=2, padx=10, pady=10)
